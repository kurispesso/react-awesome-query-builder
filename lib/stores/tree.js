...
var removeRule = function removeRule(state, path, config) {
  ...
  if (isEmptyGroup) {
    if (isParentRuleGroup) {
      var canDeleteRuleGroupIfEmpty = true;
...
var setProperty = function setProperty(state, path, property, value) {
  return state.setIn((0, _treeUtils.expandTreePath)(path, "properties", property), value);
};
/**
 * @param {Immutable.Map} state
 * @param {Immutable.List} path
...
var setField = function setField(state, path, newField, config) {
  ...
  var _config$settings = config.settings,
      ...
      unsafePropertiesRuleGroup = _config$settings.unsafePropertiesRuleGroup;
  ...
  if (isRuleGroup) {
    var defaultUnsafePropertiesRuleGroup = {};

    if (unsafePropertiesRuleGroup && currentProperties) {
      unsafePropertiesRuleGroup.map(function (unsafePropertyRuleGroup) {
        var valueUnsafePropertyRuleGroup = currentProperties.get(unsafePropertyRuleGroup);
    ...
    var groupProperties = (0, _defaultUtils.defaultGroupProperties)(config).merge(_objectSpread(_objectSpread({}, defaultUnsafePropertiesRuleGroup), {}, {
      field: newField
    }));
  ...
  return function () {
    ...
    switch (action.type) {
      ...
      case constants.SET_PROPERTY:
        return Object.assign({}, state, _objectSpread({}, unset), {
          tree: setProperty(state.tree, action.path, action.property, action.value)
        });
